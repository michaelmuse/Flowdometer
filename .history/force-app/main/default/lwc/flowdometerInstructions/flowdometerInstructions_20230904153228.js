...
export default class FlowdometerInstructions extends LightningElement {
  ...
  handleStepCompleted(event) {
    ...
    this.currentModalContent = newSteps[stepIndex].content
      ? encodeURI(newSteps[stepIndex].content)
      : newSteps[stepIndex].description;

    // Null check before updating the modal component
    const modalComponent = this.template.querySelector("c-modal");
    if (modalComponent) {
      modalComponent.currentModalContentIsIframe =
        !!newSteps[stepIndex].content; // Update the flag in the modal component
    }
