...
def extract_method_signatures(code, methods_to_search):
    method_matches = re.finditer(r"(?P<signature>((@isTest\s+)?(public|private|protected|global)\s+(static)?\s*(?P<return_type>[\w.<>]+)\s+(?P<return_var>[a-zA-Z_]\w+)\s*\(.*?\)\s*{))", code, re.DOTALL)
    extracted_methods = []
