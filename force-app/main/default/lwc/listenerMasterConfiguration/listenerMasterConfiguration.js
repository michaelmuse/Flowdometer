...
export default class ListenerMasterConfiguration extends NavigationMixin(
    ...
    async checkAuthStatus() {
        ...
        try {
            ...
            this.needsAuth = !result.connected;
        } catch (error) {
            console.error('Error checking auth status:', error);
            this.needsAuth = true; // Default to showing auth if check fails
        } finally {
            this.isLoading = false;
        }
    }
    ...
    handleAuthSuccess() {
        ...
        // Re-load SObjects now that we are authenticated
        // This will trigger the @wire(getAllSObjectsInOrg) to refresh if needed, 
        // but we might need to call it manually or just let the @wire handle it.
    }
